...
const config = {
  libraryName: 'ngx-eventbus',
  unscopedLibraryName: 'ngx-eventbus',
  allSrc: 'src/**/*',
  allTs: 'src/**/!(*.spec).ts',
...
gulp.task('build:demo', () => {
  return execDemoCmd(`build --preserve-symlinks --prod --base-href /ngx-eventbus/ --deploy-url /ngx-eventbus/`, { cwd: `${config.demoDir}`});
});
...
gulp.task('build:demo-ssr', () => {
  return execDemoCmd(`build --preserve-symlinks --prod`, { cwd: `${config.demoDir}`})
    .then(() => execDemoCmd(`run ngx-eventbus-demo:server`, { cwd: `${config.demoDir}` }))
